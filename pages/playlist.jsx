  ...
  useEffect(() => {
    async function fetchPlaylist() {
      if (data) {
        ...
        const res = await fetch(pldata.items[2].href, {
          headers: new Headers({
            Authorization: `Bearer ${data.user.accessToken}`,
          }),
        });
        const data2 = await res.json();
        setTracks(data2.tracks.items);
      ...
      <main className="w-full pt-6 min-h-[90vh] bg-backgroundColor">
        <div className="flex gap-20 m-auto p-16 justify-between max-w-[85vw] h-[80vh] bg-playlistBg rounded-lg">
          <div className=" w-full overflow-y-scroll pr-4">
            {tracks.map((m, i) => {
              return <List music={m} key={i} />;
            })}
          </div>
          <div className="p-8 w-[500px] h-[460px] bg-playlistsong rounded-lg">
            <div className="h-[250px] w-[300px] rounded-2xl bg-secondaryColor overflow-hidden">
              <Image
                src={tracks[0]?.track.album.images[0].url}
                width={640}
                height={240}
                alt=""
              />
            ...
            <div className=" my-2 text-center">
              {tracks && (
                <>
                  <h2 className=" text-2xl">{tracks[0]?.track.name}</h2>
